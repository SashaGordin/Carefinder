...
export default function MsgTemplateMVP({ passData, hasArchives }) {
  ...
  async function getMessageAvatar(theSenderID) {
    ...
    try {
      ...
      if (userData) {
        ...
        if (!profilePicPath) {
          profilePicPath = 'genericUser.jpg';
        }
  ...
  const sendReply = async (
    ...
    if (userRole === 'provider') {
      ...
      replyAppend +=
        '<br />Reply below and/or you may use any of these links anytime.';
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          if (Object.prototype.hasOwnProperty.call(userData, 'CalendlyLink')) {
            const calendlyLink = userData.CalendlyLink;
            console.log('Provider calendlyLink:', calendlyLink);
            replyAppend +=
              '<br />📆 <b><a href="' +
              calendlyLink +
              '" target="_blank">SCHEDULE MEETING</a></b>';
          } else {
          ...
          if (
            ...
            if (assessmentFileName) {
              const fileRef = firebase
                .storage()
                .ref()
                .child(`assessments/${assessmentFileName}`);
              try {
                const url = await fileRef.getDownloadURL();
                replyAppend +=
                  '<br />Reply to this user below. As a reminder, the user has an ';
                replyAppend +=
                  'assessment on file: 📁 <b><a href="' +
                  url +
                  '" target="_blank">DOWNLOAD</a></b>';
                replyText += replyAppend;
              ...
              replyAppend +=
                '<br />(⚠️ User has no assessment on file with CareFinder. You should ask the user to upload or schedule one.)';
              replyText += replyAppend;
            }
          }
        }
    ...
    if (userRole === 'admin') {
      ...
      replyAppend +=
        '<br />This message is from a CareFinder admin. Let us know if you have any additional questions.';
      replyText += replyAppend;
    }
  ...
  return (
    <>
      <div className="messagesDesktopUX">
        {' '}
        {/* THIS IS THE DESKTOP UX ... */}
        <div className="msgDesktopLeft">
          {' '}
          {/* LEFT SIDE OF DESKTOP UX */}
          {hasArchives ? (
            <>
        ...
        {/* msgDesktopLeft */}
        <div className="msgDesktopRight">
          {' '}
          {/* RIGHT SIDE OF DESKTOP UX, where MESSAGE SHOW HERE... */}
          {passData.map((thisMsg, index) => (
            <>
              {activeDivId === thisMsg['m_ID'] && (
                <>
                  <div className="rightFromHeader">
                    ...
                    <div className="msgAlertRight"></div>
                  </div>
      ...
      {/* messagesDesktopUX */}
      <div className="clear"></div>
      <div className="messagesMobileUX">
        {' '}
        {/* MOBILE UX IN THIS DIV... */}
        <>
          {hasArchives ? (
            <>
              <div className="msgTopControls">
