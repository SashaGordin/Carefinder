...
export default function AdminViewClient() {
  ...
  const decryptPDF = async () => {
    try {
      ...
      const decipherKey = forge.cipher.createDecipher(
        'AES-CBC',
      ...
      const decipherIV = forge.cipher.createDecipher(
        'AES-CBC',
        NODE_FORGE_MASTERKEY_HEX
      );
      ...
      decipherFile.finish();
      const decryptedContentBuffer = Buffer.from(
        decipherFile.output.getBytes(),
        'binary'
      );
