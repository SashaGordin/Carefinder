...
const FileUpload = ({
  ...
  const handleUploadFiles = () => {
    ...
    for (const file of fileList) {
      ...
      if (!acceptedFileTypes) isAcceptedFileType = true;
      else if (acceptedFileTypes.endsWith('*'))
        isAcceptedFileType = file.type.startsWith(
          acceptedFileTypes.replace('*', '')
        );
      else {
        ...
        for (let type of arr) {
          if (type.startsWith('.'))
            //extension
            isAcceptedFileType |= file.name.endsWith(type); //mime type
          else isAcceptedFileType |= file.type === type;
        }
      }
      ...
      if (isAcceptedFileType) {
        ...
        console.log(
          'FILE REJECTED: You cannot upload ' + file.type + ' files. Kthx.'
        );
      }
    }
