			...
			if (acceptableMimeTypes.includes(file.type)) {
				...
				try {
					...
					if (fileData) {
						...
						const encryptedDataBase64 = await encryptFile(fileData, singleKey, singleIV);
						...
						try {
							...
							const cipherKey = forge.cipher.createCipher('AES-CBC', NODE_FORGE_MASTERKEY_HEX);
							...
							const cipherIV = forge.cipher.createCipher('AES-CBC', NODE_FORGE_MASTERKEY_HEX);
							...
							try {
								...
								alert('AWESOME! We received the file and have saved it securely (and encrypted) for our review. You may click `Next` and go to the next screen. Thanks!');
