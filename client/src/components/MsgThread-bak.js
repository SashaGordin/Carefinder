...
//v20240430.837pm
// WORKING BUT NOT SUBSCRIBED TO CHANGES VIA ONSHAPSHOT
...
function MsgThread({ threadID, pageIteration }) {
  ...
  const fetchThread = async () => {
    ...
    const threadMessages = await dbCollection
      .orderBy('msgDate', 'asc')
    ...
    if (!threadMessages.empty) {
      ...
      threadMessages.forEach((thisThreadItem) => {
        if (!localThreadArray[localThreadIndex]) {
          localThreadArray[localThreadIndex] = [];
        }

        localThreadArray[localThreadIndex]['mt_ID'] = thisThreadItem.id;
        localThreadArray[localThreadIndex]['mt_FR'] =
          thisThreadItem.data()['msgFrom'];
        localThreadArray[localThreadIndex]['mt_TX'] =
          thisThreadItem.data()['msgText'];
        localThreadArray[localThreadIndex]['mt_TH'] =
          thisThreadItem.data()['msgThreadID'];
        localThreadArray[localThreadIndex]['mt_TO'] =
          thisThreadItem.data()['msgTo'];
        localThreadArray[localThreadIndex]['mt_DS'] =
          thisThreadItem.data()['msgDate']['seconds'];
      ...
      {entireThread.map((item, index) => (
        <div key={index}>
          {item.mt_FR === currentUser.uid ? (
            <>
              <div className="msgThreadDate msgThreadDateRight">
                {item.mt_DA}
              </div>
            ...
            <>
              <div className="msgThreadDate msgThreadDateLeft">
                {item.mt_DA}
              </div>
