...
export default function AssociatedCoCNotIncluded({
  listingInfo,
  setListingInfo,
  handleUpdate,
}) {
  ...
  const fullFeatureList =
    listingInfo.associatedCoCNotIncluded?.fullFeatureList ??
    defaultNotIncludedFeatureList;
          ...
          <Col xs={8}>
            <Card>
              ...
              <div style={{ height: '400px', overflowY: 'auto' }}>
                {listingInfo.associatedCoCNotIncluded?.selectedFeaturesNotIncluded?.map(
                  (featureId, i) => (
                    <div className={'small'} key={i}>
                      {fullFeatureList[featureId]}
                    </div>
                  )
                )}
              </div>
            ...
            <Card>
              ...
              >
                <ToggleButtonGroup
                  className="m-auto"
                  type="checkbox"
                  name="selectedFeaturesNotIncluded"
                  vertical
                  value={
                    listingInfo.associatedCoCNotIncluded
                      ?.selectedFeaturesNotIncluded
                  }
