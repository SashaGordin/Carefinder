  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (error.code === 'auth/weak-password') {
        ...
        console.log(
          'Attempting to find user ID for email:',
          emailRef.current.value
        );
        ...
        try {
          ...
          if (!querySnapshot.empty) {
            ...
            console.log(
              'Email address is already in use by user with ID: ' + userID
            );
          } else {
            console.log(
              `Email address is already in use, but it's not in Firebase`
            );
          }
        } catch (queryError) {
      ...
      <div className="contentContainer utilityPage loginPage">
        <Card>
          <Card.Body>
            <h2 className="text-center mb-4">
              Sign Up to Find an Adult Family Home
            </h2>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              {isTouched && (
                <>
                  <div
                    className="loginPasswordControl"
                    onClick={togglePasswordVisibility}
                    style={{ cursor: 'pointer' }}
                  >
                    {showPassword ? (
                      <>
                        <FaEyeSlash /> Hide Passwords
                      </>
                    ) : (
                      <>
                  ...
                  <div className="loginPasswordControl">
                    {!passwordsMatch ? (
                      <>
                        <span className="CFred">
                          <FaTimes /> Passwords do not match
                        </span>
                      </>
                    ) : (
                      <>
                        <span className="CFgreen">
        ...
        {fromClaimProfile ? null : (
          <>
            ...
            <div className="w-100 text-center mt-2">
              If you are with an Adult Family Home{' '}
              <Link to="/claim-profile">
                sign up here as licensed AFH care provider
              </Link>
              .
            </div>
          </>
