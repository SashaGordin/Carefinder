...
export default function ReservationResponse() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_ENDPOINT}/getUser`,
          {
    ...
    const fetchProviderData = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_ENDPOINT}/getProvider`,
          {
  ...
  const onAccept = async () => {
    try {
      ...
      const res = await axios.post(
        `${process.env.REACT_APP_ENDPOINT}/charge-customer`,
        {
          amount: 150000,
      ...
      await axios.post(
        `${process.env.REACT_APP_ENDPOINT}/create-subscription`,
        {
      ...
      if (res.status === 200) {
        const messageText =
          'CONGRATS! Your room is reserved and we have charged your CC.';
        ...
        console.log(
          'Room has been reserved and customer charged successfully.'
        );
      } else {
  ...
  const onDeny = async () => {
    try {
      const messageText =
        'SORRY, we cannot approve your request at this time, and your CC was not charged.';
      const dbCollection = firestore.collection('messages');
      await dbCollection.add({
        msgDate: Timestamp.now(),
        msgTo: messageSeniorID,
      ...
      <Card>
        <Card.Body>
          ...
          <Card.Text>
            ...
            senior resident {senior.displayName} for the move-in day of (date).
            If you accept, you will receive a non-refundable deposit of $1,500
            to hold the room for this resident. View terms.
          </Card.Text>
