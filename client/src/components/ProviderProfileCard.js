import React, { useState } from "react";
import { Card, Button, Modal, Badge } from "react-bootstrap";
import PropertyPhotoModal from "./PropertyPhotoModal";
import { useNavigate } from "react-router-dom";
import VisitingMap from "./VisitingMap";

const ProviderProfileCard = ({ provider }) => {
	const {
		profilePicPath,
		FacilityPOC,
		FacilityName,
		homePhotos,
		listingsData, // Add more fields as needed
		Speciality,
	} = provider;
	console.log("adskfjslkdjf", provider);

	const roomPhotos = listingsData[0].roomData.map((room) => room.roomPhotos);

	const [showModal, setShowModal] = useState(false);

	const [showImageModal, setShowImageModal] = useState(false);

	const navigate = useNavigate();

	const handleImageClick = () => {
		setShowImageModal(true);
	};

	const handleClose = () => setShowModal(false);
	const handleShow = () => setShowModal(true);

	console.log("this is the provider", provider);
	return (
		<>
			<div
				className="provider-card"
				data-license-number={provider.LicenseNumber}
				style={{
					display: "flex",
					flexDirection: "row",
					flex: 1,
					margin: "5px",
				}}
			>
				<img className="w-40" src={profilePicPath} alt="Profile pic" />
				<div className="flex flex-col">
					<Card.Title>{FacilityPOC}</Card.Title>
					<div>5 miles away</div>
					<Button className="text-xs" onClick={handleShow}>
						View profile
					</Button>
				</div>
			</div>
			<Modal
				style={{ color: "#ffffff" }}
				show={showModal}
				onHide={handleClose}
				size="xl"
				centered
			>
				<Modal.Body
					style={{
						backgroundColor: "#232323",
						color: "#ffffff",
						padding: "40px",
					}}
				>
					<div
						style={{ display: "flex", gap: "20px", cursor: "pointer" }}
						onClick={handleImageClick}
					>
						<img
							style={{ width: "40%", borderRadius: "10px", objectFit: "cover" }}
							src={profilePicPath}
							alt="Profile pic"
						/>
						<div
							style={{
								display: "grid",
								gridTemplateColumns: "1fr 1fr",
								gap: "10px",
								width: "60%",
							}}
						>
							{homePhotos.slice(0, 4).map((photo, index) => (
								<img
									key={index}
									style={{
										width: "100%",
										height: "100%",
										borderRadius: "10px",
										objectFit: "cover",
									}}
									src={photo}
									alt={`Photo ${index + 1}`}
								/>
							))}
						</div>
					</div>
					<div
						style={{
							display: "flex",
							justifyContent: "space-between",
							marginTop: "20px",
						}}
					>
						<div>
							<div style={{ fontSize: "24px", fontWeight: "bold" }}>
								{FacilityPOC}
							</div>
							<div
								style={{
									fontSize: "20px",
									fontWeight: "bold",
									marginBottom: "10px",
								}}
							>
								{FacilityName}
							</div>
							<Badge
								pill
								variant="secondary"
								style={{
									backgroundColor: "#2C2C2C",
									color: "white",
									marginRight: "10px",
								}}
							>
								Verified Provider
							</Badge>
							<Badge
								pill
								variant="secondary"
								style={{ backgroundColor: "#2C2C2C", color: "white" }}
							>
								Licensed since 1998
							</Badge>
							<div style={{ marginTop: "10px" }}>Licensed For:</div>
							<div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
								<Badge
									pill
									variant="secondary"
									style={{ backgroundColor: "#2C2C2C", color: "white" }}
								>
									Mental Health
								</Badge>
								<Badge
									pill
									variant="secondary"
									style={{ backgroundColor: "#2C2C2C", color: "white" }}
								>
									Dementia
								</Badge>
								<Badge
									pill
									variant="secondary"
									style={{ backgroundColor: "#2C2C2C", color: "white" }}
								>
									Developmental Disabilities
								</Badge>
							</div>
						</div>
						<div
							style={{
								display: "flex",
								flexDirection: "column",
								justifyContent: "center",
								alignItems: "flex-end",
							}}
						>
							<Button
								style={{
									marginBottom: "10px",
									backgroundColor: "#000",
									border: "none",
									width: "140px",
								}}
								onClick={() =>
									(window.location.href = `/msg-outbox?ref=${provider.userId}`)
								}
							>
								Message
							</Button>
							<Button
								style={{
									backgroundColor: "#FF69B4",
									border: "none",
									width: "140px",
								}}
								onClick={() =>
									navigate("/reserve-room", { state: { provider } })
								}
							>
								Reserve Room
							</Button>
						</div>
					</div>
					<hr style={{ borderColor: "#777", margin: "20px 0" }} />
					<div>
						<h4 style={{ color: "#ffffff" }}>What's special</h4>
						<div>
							At our Adult Family Home, we provide personalized care in a warm
							and welcoming environment...
						</div>
						<h4 style={{ color: "#ffffff", marginTop: "20px" }}>Features</h4>
						<div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
							<Button
								variant="outline-light"
								style={{
									backgroundColor: "#2C2C2C",
									borderRadius: "10px",
									color: "#ffffff",
								}}
							>
								Big Spacious
							</Button>
							<Button
								variant="outline-light"
								style={{
									backgroundColor: "#2C2C2C",
									borderRadius: "10px",
									color: "#ffffff",
								}}
							>
								Women-only
							</Button>
							<Button
								variant="outline-light"
								style={{
									backgroundColor: "#2C2C2C",
									borderRadius: "10px",
									color: "#ffffff",
								}}
							>
								Pet accepted
							</Button>
						</div>
						<h4 style={{ color: "#ffffff", marginTop: "20px" }}>
							Where will you be visiting from?
						</h4>
						<VisitingMap provider={provider} />
					</div>
					<div
						style={{
							backgroundImage: 'url("bookCare.png")',
							borderRadius: "8px",
							padding: "20px",
						}}
					>
						<div style={{ color: "#ffffff", fontSize: "50px" }}>Book Care.</div>
						<div>
							Review disclosures, contract, make a deposit of first months rent
							outlined in quote, select a move in date and you're ready to move.
						</div>
						<div
							style={{
								display: "flex",
								flexDirection: "row",
								justifyContent: "space-between",
								width: "100%",
							}}
						>
							<div></div>
							{/* Use marginTop: auto to push the button to the bottom */}
							<Button>Reserve Room</Button>
						</div>
					</div>
				</Modal.Body>
			</Modal>

			{showImageModal && (
				<PropertyPhotoModal
					showModal={showImageModal}
					setShowModal={setShowImageModal}
					FacilityName={FacilityName}
					homePhotos={homePhotos}
					roomPhotos={roomPhotos}
				/>
			)}
		</>
	);
};

export default ProviderProfileCard;
