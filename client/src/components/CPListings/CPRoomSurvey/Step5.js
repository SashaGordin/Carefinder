...
export default function Step5({ roomInfo, setRoomInfo }) {
  const getInitialOptionState = () => {
    if (roomInfo.addlDetails) {
      for (const val of roomInfo.addlDetails) {
        if (val.startsWith('Other')) return val.substring(5);
      }
    }
    return '';
  };
  ...
  const getCheckedStatus = (option) => {
    if (roomInfo.addlDetails?.includes(option)) return true;
    else if (option === 'Other' && roomInfo.addlDetails) {
      for (const val of roomInfo.addlDetails) {
        if (val.startsWith('Other')) return true;
      }
    }
    return false;
  };
  ...
  const handleChange = () => {
    ...
    document.querySelectorAll("[type='checkbox']:checked").forEach((t) => {
      let thisVal = t.id.startsWith('Other')
  ...
  const options = [
    'Main floor',
    'Must use stairs or elevator to access room',
    'Door to outside',
    'Pleasant view',
    'Other',
  ];
