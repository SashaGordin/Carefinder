  ...
  return (
    <>
      ...
      <div className="contentContainer utilityPage">
        ...
        <p>
          Here you (an Admin) can view a client profile privately, including
          downloading an assessment if available.
        </p>
        ...
        <button
          variant="primary"
          onClick={() => getUserRecord(userId)}
          style={{ padding: 9, position: 'relative', top: -3 }}
        >
          Get User Record
        </button>
        ...
        {userRecord && (
          <div>
            <hr></hr>
            <p>
              NOTE: We can tie this into the outgoing message system so that
              admins can use this screen to send messages to the viewed user.
            </p>

            {userRecord.profilePicPath && (
              <img
                alt={''}
                src={userRecord.profilePicPath}
                style={{ width: 300 }}
              />
            )}
            <h3>
              User Record for{' '}
              <span className="CForange">{userRecord.displayName}</span>:
            </h3>

            {createdAt && (
              <>
                <p>
                  <b>Creation Date</b>: {createdAt.toLocaleString()}
                </p>
              </>
            )}
            ...
            {userRecord.TelephoneNmbr && (
              <>
                <p>
                  <b>Phone</b>: {userRecord.TelephoneNmbr}
                </p>
              </>
            )}
            ...
            {userRecord.assessmentPDFfileName && (
              <>
                <button onClick={viewPDF}>
                  Download and View Assessment PDF
                </button>
              </>
            )}
            ...
            <h3>
              Send this user a message via our messaging system (will show in
              your inbox):
            </h3>
            <textarea
              id="messageTextArea"
